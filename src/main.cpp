      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          for (int i=0; i<ptsx.size(); i++) {
            lptsx[i] = (ptsx[i] - px) * cos(psi)    + (ptsy[i] - py) * sin(psi);
            lptsy[i] = -1*(ptsx[i] - px) * sin(psi) + (ptsy[i] - py) * cos(psi);
          }
          ...
          */
          ...
          // Lets get the polynomial fitted to waypoints (yellow line)
          for (int i=0; i<50; i++) {
          ...
          //msgJson["next_x"] = lptsx;
          //msgJson["next_y"] = lptsy;
